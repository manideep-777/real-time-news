...
client_together = Together(api_key="dde59774358b86a8526b76ae9fd9b840cdbc140d97f79974466b80d5f896a630")
...
def check_if_issue(article):
    ...
    try:
        ...
        time.sleep(2)  # avoid hammering the free API
        answer = response.choices[0].message.content.strip()

        if answer.upper().startswith("YES"):
            reason = answer[3:].strip()  # remove "YES"
            return True, reason or "No reason provided"
        return False, ""
...
def summarize_news_by_date():
    ...
    # Call your internal route to fetch articles
    recent_response = app.test_client().get(f"/published-articles-by-date?date={date_str}")
    if recent_response.status_code != 200:
        return jsonify({"status": "error", "message": "Failed to fetch articles"}), 500

    recent_json = recent_response.get_json()
    if recent_json.get("status") != "success" or not recent_json.get("articles"):
        return jsonify({"status": "error", "message": "No valid articles found"}), 404

    articles = recent_json["articles"]
    if not articles:
        return jsonify({"status": "error", "message": "No articles to summarize"}), 400

    # Trimmed content for token efficiency
    combined_content = ""
    for art in articles:
        headline = art.get("headline", "").strip()
...
"""
    ...
    try:
        ...
        if not json_text:
            return jsonify({"status": "error", "message": "Model did not return valid JSON"}), 500
...
def fetch_news():
    ...
    try:
        ...
        for art in articles:
            ...
            # Translate non-English fields using Together AI
            # if is_not_english(headline_raw):
            #     headline_raw = translate_text(headline_raw)
            # if is_not_english(description_raw):
            #     description_raw = translate_text(description_raw)
            # if is_not_english(content_raw):
            #     content_raw = translate_text(content_raw)
...
def summarize_news():
    ...
    for art in articles:
        headline = art.get("headline", "").strip()
        issue_reason = art.get("issue_reason", "N/A").strip()
...
1. Extract **major public issues** strictly related to Andhra Pradesh.
2. Group into **exactly 8 mandatory categories**:
   - Governance, Law & Order, Healthcare, Infrastructure, Education, Environment, Corruption & Politics, Agriculture
3. For each category:
   - Provide **at least 4 distinct issues** (more if needed)
   - Summarize each in 1–2 factual sentences
   - Include **exact location** (district/city/town)

Rules:
- Don’t fabricate or generalize.
- No entertainment/sports topics.
...
{{
  "categories": [
    {{
      "category_name": "Agriculture",
      "issues": [
        "Farmers in XYZ District reported ...",
        ...
      ]
    }},
    ...
...
"""
    ...
    try:
        response = client_together.chat.completions.create(
            model="meta-llama/Llama-3.3-70B-Instruct-Turbo-Free",
            messages=[
                {"role": "system", "content": "You are a structured JSON news summarizer."},
                {"role": "user", "content": prompt}
            ],
            max_tokens=2048,
            temperature=0.3
